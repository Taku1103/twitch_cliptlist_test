class Api::PlaylistsController < ApplicationController
  ...
  def index
    ...
    render json: { status: "getting playlists sucessed" , playlists: @playlists}
  end

  def create
    @playlist = Playlist.new(playlist_param)
    if @playlist.save
      render json: { status: "Creating playlist sucessed" , playlist: @playlist }
    else
      render json: { status: "Creating playlist failed"}
    end
  end
